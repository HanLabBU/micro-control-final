...
// try to add index params
volatile float idleIndex = 1;
volatile float idleTh = 10;
volatile float idleMax;
// =============================================================================
//   SETUP & LOOP
// =============================================================================
...
static inline void beginAcquisition(char input[], int8_t length) {
    ...
    char *idle_period_s = strtok(NULL,",");
    float idle_period_s_int = atof(idle_period_s);
...
//    float water_jitter_s_int = atof(water_jitter_s);
    ...
    idleMax = floor(idle_period_s_int*1000.0/sampling_interval_ms_int);
...
void captureDisplacement() {
  ...
  // check if mouse has been idale
  float dx = abs(currentSample.left.p.dx) + abs(currentSample.right.p.dx);
  float dy = abs(currentSample.left.p.dy) + abs(currentSample.right.p.dy);
  //Serial.print(dx,dy);

  if (dx < idleTh*2 || dy < idleTh*2) {
    idleIndex++;
  }
  else {
    idleIndex = 1;
  ...
  // Send Data
  sendData(currentSample,idleIndex);
  currentFrameTimestamp = millisSinceAcquisitionStart;
...
void sendHeader() {
  ...
  Serial.print(String(
      ...
      delimiter + flatFieldNames[5] + " [" + tunit + "]" +delimiter + " idleIndex " "\n"));
}

void sendData(sensor_sample_t sample, float idleIndex) {
    ...
    const String waterPinVal = String(idleIndex);
