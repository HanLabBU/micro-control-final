...
huiw6 = uicontrol('Style','edit', 'Units', 'Normalized', ...
    'Position', [0.35 0.13 0.33 0.05],'string','Idle period before puff? [s]',...
    'FontName','Wawati SC', 'FontSize',9);
...
function callbackfn1(~,~)
    ...
    fwrite(uart,sprintf('%s,%s,%s,%s',huiw4.String, huiw5.String, huiw6.String,'bla'));
    pause(0.1);
    nreps = str2double(fscanf(uart,'%s\n'));
    repcycles = fscanf(uart,'%s\n');
%    water_jitter = fscanf(uart,'%s\n');
    idle_time = fscanf(uart,'%s\n');

    movement = cell(nreps,1);
    fprintf('nreps: %d, repcycles: %s, idle: %s, \n',nreps,repcycles,idle_time);
