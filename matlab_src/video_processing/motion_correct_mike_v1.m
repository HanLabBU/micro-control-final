...
function [err] = motion_correct_mike_v1(filename, suffix, timespan, pref) 
...
%% motion correct each file and save it with 'm_' at the beginning for raw  data and 'm_f_' for homomorphic filtered version
    ...
    for n=1:size(file_frames,1)
        ...
        if n == 1
            [data, procstart_m_f.hompre] = homomorphicFilter(data);
            [data, procstart_m_f.xc,procstart_m_f.prealign] = correctMotion(data);
        else
            [data, procstart_m_f.hompre] = homomorphicFilter(data,procstart_m_f.hompre);
            [data, procstart_m_f.xc,procstart_m_f.prealign] = correctMotion(data,procstart_m_f.prealign);
        end
        
        if n == 1
            [data, procstart_m_f.norm] = normalizeData2(data);
        else
            [data, procstart_m_f.norm] = normalizeData2(data,procstart_m_f.norm);
        end
        
        %mike added this piece
        save_filename = [fold 'm_f_n2_',suffix,'_',pref];
        save([fold 'procstartdata' suffix '_' pref '_' num2str(n) '.mat'],'procstart_m_f');
        matrix2tiff(data, currinfo, [save_filename '_' num2str(n)], 'w');
        clear data
    end
