...
hold on;
plot(t_true(subsampleinds),mdl.predict(t_true(subsampleinds)'),'-r','LineWidth',2);
hold off;
...
figure;
plt1 = plot(t_true,camera_on_times(:),'r','LineWidth',1);
hold on;
plt2 = plot(t_true,t_true,'b','LineWidth',1);
plt2.Color(4) = 0.5;
hold off;
xlabel('Theoretical time[s]');
ylabel('Measured time [s]');
print('figures/predicted_vs_theoretical_teensy_v3.svg','-dsvg');

%%
plt2.Color(4) = 1;
xlim([0 .05])
ylim([0 .05])
print('figures/predicted_vs_theoretical_teensy_v3_early.svg','-dsvg');

%%
xlim([599.9 599.95])
ylim([599.9 599.95])
print('figures/predicted_vs_theoretical_teensy_v3_late.svg','-dsvg');
